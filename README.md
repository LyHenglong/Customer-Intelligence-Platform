...
2. **Migrate the data** the dashboard actually reads (`marts.customer_360` plus the small operational tables — deliberately *not* the raw/staging pipeline tables, which the dashboard never queries and would roughly double the transfer):
   ...
   ```
   Verified end to end against a throwaway local database before ever writing this down: all 1,000,000 `customer_360` rows, 228 drift rows, 13 ingestion-log rows round-tripped correctly (237MB on disk).
...
`src/warehouse.py` and the dashboard's own `get_pg_conn()` both connect with `sslmode="prefer"` (configurable via `POSTGRES_SSLMODE`), so the identical code reaches both the local Docker Postgres (no SSL) and Neon (SSL required) without an environment-specific branch — see Deviations for the two real bugs this deployment path surfaced.
...
- **The Streamlit Cloud deployment path was verified as thoroughly as it's possible to without an actual Neon account** (this session has no cloud credentials of any kind - see Deviations). `scripts/migrate_to_neon.sh`'s dump/restore was run for real against a throwaway local Postgres database standing in for the target: all 1,000,000 `customer_360` rows, all 228 `feature_drift` rows, and every operational table round-tripped correctly, verified by direct row-count and spot-check queries afterward (including the exact customer used in the AI Agent Layer's README examples). `requirements-streamlit-cloud.txt` was installed into a clean virtualenv (catching the concurrent-install bug below) and the dashboard was run under `streamlit run` against it - real 1,000,000-row KPIs rendered correctly, and two real bugs surfaced and were fixed this way that a plain `import` check would have missed entirely (see Deviations). What remains genuinely unverified: an actual SSL handshake against a real Neon endpoint, and Neon free-tier storage limits against the real 237MB payload - both require an account this session cannot create.
...
- **The public demo deployment (Streamlit Cloud + Neon) is prepared and tested as far as this session's tooling allows, but not against a real hosted account** - this environment has no cloud credentials of any kind (checked explicitly: no AWS/GCP/Azure/Fly/Railway/Render CLI or tokens). The migration script, the trimmed dependency set, and the dashboard code itself were all verified against real substitutes (a throwaway local Postgres database, a clean virtualenv, a real `streamlit run`), and two real bugs surfaced and were fixed this way - but an actual SSL handshake against Neon's real endpoint and Neon's free-tier storage limit against the real 237MB payload remain genuinely untested. Account creation (Neon, Streamlit Cloud) is the one part of this only a human can do.
