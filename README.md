...
- **pytest**: 63/63 pass locally, and the same `pytest tests/` command is green on GitHub Actions — most recently on commit `PENDING_COMMIT_HASH`, the commit that added the recommender-evaluation and calibration-regression tests, with the dependency-install step (lightgbm, shap, dill) succeeding on a clean Ubuntu runner. Covers ingestion logic, expected-value/threshold math, recommender invariants, PSI drift, recommender evaluation metrics, and the calibration/SHAP artifact-shape regression.
